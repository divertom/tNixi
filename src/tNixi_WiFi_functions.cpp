...
/*
 ...
 if (WiFiConnected)
    {
        //Do all the suff here that requires WiFi

        //configTime(TimeOffsetGMT, DST_OFFSET, NTP_SERVER);

        //Take care of internet time
        if (!NTPClientInitialized)
        {
            NTPTimeClient.begin();
            NTPTimeClient.update();
            Serial.println(NTPTimeClient.getFormattedTime());
            NTPClientInitialized = true;
        }
        else
        {
            NTPTimeClient.update();
